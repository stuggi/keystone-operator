...
func (th *TestHelper) CreateKeystoneEndpoint(name types.NamespacedName) types.NamespacedName {
	...
	// the Status field needs to be written via a separate client
	endpoint = th.GetKeystoneEndpoint(name)
	endpoint.Status = keystonev1.KeystoneEndpointStatus{
		EndpointIDs: map[string]string{
			"internal": uuid.New().String(),
			"public":   uuid.New().String(),
		},
		ServiceID: uuid.New().String(),
		Endpoints: []keystonev1.Endpoint{
			{
				ID:        endpoint.Status.EndpointIDs["internal"],
				Interface: "internal",
				URL:       endpoint.Spec.Endpoints["internal"],
			},
			{
				ID:        endpoint.Status.EndpointIDs["public"],
				Interface: "public",
				URL:       endpoint.Spec.Endpoints["public"],
			},
		},
	}
	endpoint.Status.ObservedGeneration = endpoint.Generation
	gomega.Expect(th.K8sClient.Status().Update(th.Ctx, endpoint.DeepCopy())).Should(gomega.Succeed())
